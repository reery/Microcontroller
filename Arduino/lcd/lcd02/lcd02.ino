...
float temp, temp1, temp2, temp_avg_10, temp_avg_30;
int i, j, counter, counter_10_1 = 0, counter_10_2 = 0, counter_30_1 = 0, counter_30_2 = 0;
int switchState;
float temp_average_10s[10];
float temp_average_30s[30];
float temp_avg_2m[12];
float temp_avg_6m[12];
int buttonPressed = 0, menu = 0, twelve_10 = 0, twelve_30 = 0;
float ten_min, ten_max, thirty_min, thirty_max;
...
// Graph icons for LCD for 0-100% bars
byte graph1[8] = {
  B00000,
  B00000,
  B00000,
...
void loop() {
  ...
  // counter_10_2 is used for calculating average value for the first 10 secs.
  // Here it increments from 0 up to 10 and stays at 10 forever.
  // NOTE: This was 9
  if(counter_10_2 < 10)
    counter_10_2++;

  // Same for counter_30_2 but for the first 30 secs.
  // NOTE: This was 29
  if(counter_30_2 < 30)
    counter_30_2++;

  // counter_10_1 is used for array position of the 10s average counter per second
  ...
  // TODO: Copy this IF to the counter_30_1 IF below.
  if(counter_10_1 > 9){
    counter_10_1 = 0;
  ...
  // TODO: Calculate min/max for temp_avg_6m.
  ten_min = calcMin(temp_avg_2m, 12);
  ten_max = calcMax(temp_avg_2m, 12);

  // Same for counter_30_1 as it was for counter_10_1.
  // TODO: Copy from counter_10_1 above.
  if(counter_30_1 > 29)
    counter_30_1 = 0;
...
// FUNCTION: Prints graphs for 10s/2m average
void printGraph(void){
  ...
  for(i = 4, j = 0; i < 16; i++, j++){
    lcd.setCursor(i, 0);
    lcd.write(byte(calcGraph(&ten_min, &ten_max, &temp_avg_2m[j])));
  }
