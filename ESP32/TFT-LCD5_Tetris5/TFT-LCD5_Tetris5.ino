...
//  Serial.println(accXint);
 
  accXpos = map(accXint,-800,800,0,6);
  Serial.print("accXpos: ");
  Serial.println(accXpos);

  if(accXpos >= 6 && xPrev == 5) {
    x = 48;
  } else if(accXpos == 5 && xPrev == 4 || xPrev == 6) {
    x = 40;
  } else if(accXpos == 4 && xPrev == 3 || xPrev == 5) {
    x = 32;
  } else if(accXpos == 3 && xPrev == 2 || xPrev == 4) {
    x = 24;
  } else if(accXpos == 2 && xPrev == 1 || xPrev == 3) {
    x = 16;
  } else if(accXpos == 1 && xPrev == 2) {
    x = 8;
  } else {
    x = 1;
  }
...
void tetrisPhysics(int x) {
  ...
  for(i = 0; i < BOTTOM - 1; i++, y++) {
    ...
    for(xCounter = 1, blockX = 0; x > xCounter; xCounter = xCounter + 8, blockX++);

    printStats(x, y, blockX, blockY);

    // x-control
    xPrev = x; // probably not needed
    xDirection = calcAccX(xPrev);
    
    // checks if the next quadrant (bottom) is occupied, and if so, the loop has to end
    // BUG: when moving too fast in x direction the block stopps inbetween lines
    //      should check for pixel instead of blocks
    //      better change the definition of when blockY is defined
    if(xDirection == 0 && blockArray[blockX - 1][blockY] >= 1 || blockX == 0) {
    } else if(xDirection == 48 && blockArray[blockX + 4][blockY] >= 1 || blockX == 6) {
    } else {
      x = xDirection;
    }
    
    if(blockArray[blockX][blockY-1] >= 1 || 
        blockArray[blockX+1][blockY-1] >= 1 ||
        blockArray[blockX+2][blockY-1] >= 1 ||
        blockArray[blockX+3][blockY-1] >= 1 && 
        y == 151 - (8 * blockY)) {
    ...
    //tft.fillRect(x, y-1, 32, 8, TFT_BLACK);
    

    if(xDirection == 8) {
      tft.fillRect(x - 8, y - 1, 8, 8, TFT_BLACK); // erase left side to reduce flickering
    } else if (xDirection == -8) {
      tft.fillRect(x + 32, y - 1, 8, 8, TFT_BLACK); // erase right side to reduce flickering
    }
    //tft.fillRect(x + xDirection, y, 1, 8, TFT_BLACK);
...
void printStats(int x, int y, int blockX, int blockY){
