...
uint8_t calcAccX(void) {
  ...
  accXpos = map(accXint,-200,200,1,49);
...
void tetrisPhysics(int x) {
  ...
  for(i = 0; i < BOTTOM - 1; i++, y++) {
    ...
    x = calcAccX();
    //Serial.println(x);
    //tft.fillRect(x, y - 1, 32, 8, TFT_BLACK); // erases top line to reduce flickering
    ...
    for(yCounter = BOTTOM - 1, blockY = 1; y < yCounter; yCounter = yCounter - 8, blockY++);
